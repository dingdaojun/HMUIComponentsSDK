...
#import "HMServiceAPIRunRopeSkippingDataItem.h"
...
{
    NSArray *ropeSkippingArray = [self hmSport_ComponentsSeparatedBySemicolon];
    
    NSMutableArray *ropeSkippingDatas = [NSMutableArray array];
    NSTimeInterval lastTime = startTime;
    
    for (NSString *str in ropeSkippingArray) {
        
        HMServiceAPIRunRopeSkippingDataItem *item = [self decodeRopeSkippingWithEncodeStr:str lastTime:lastTime];
        if (!item) {
            continue;
        }
        [ropeSkippingDatas addObject:item];
        lastTime = item.time;
    }
    
    return ropeSkippingDatas;
}

- (HMServiceAPIRunRopeSkippingDataItem *)decodeRopeSkippingWithEncodeStr:(NSString *)ropeSkippingEncodeStr lastTime:(NSTimeInterval)lastTime
{
    HMServiceAPIRunRopeSkippingDataItem *item = [[HMServiceAPIRunRopeSkippingDataItem alloc] init];
    NSArray *ropeSkippingArray = [ropeSkippingEncodeStr componentsSeparatedByString:@","];
    
    if ([ropeSkippingArray count] < 2) {
        return nil;
    }
    
    item.time = [ropeSkippingArray[0] doubleValue] + lastTime;
    item.ropeSkippingFrequency = [ropeSkippingArray[1] integerValue];
...
- (HMServiceAPIRunLapDataItem *)decodeLapWithEncodeStr:(NSString *)lapEncodeStr
    ...
    if ([paceArray count] < 20) {
        return item;
    }
    
    item.ropeSkippingCount = [paceArray[19] integerValue];
