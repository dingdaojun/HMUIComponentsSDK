...
static HMServiceAPIRunSourceType runSourceTypeWithString(NSString *string) {
    NSDictionary *map = @{@"run.mifit.huami.com" : @(HMServiceAPIRunSourceTypeMifit),
                          ...
                          @"run.dongtinghu.huami.com" : @(HMServiceAPIRunSourceTypeDongtinghu),
                          };
...
static NSString *stringWithRunSourceType(HMServiceAPIRunSourceType type) {
    switch (type) {
        ...
        case HMServiceAPIRunSourceTypeDongtinghu: return @"run.dongtinghu.huami.com";
    }
}
...
- (NSArray<id<HMServiceAPIRunRopeSkippingFrequencyData>> *)api_runDetailDataRopeSkippingFrequency {
    NSTimeInterval startTime = self.hmjson[@"trackid"].doubleValue;
    NSString *ropeSkippingEncodeStr = self.hmjson[@"rope_skipping_frequency"].string;
    
    return [ropeSkippingEncodeStr hm_stringByDecodingRopeSkippingWithStartTime:startTime];
}
...
- (NSInteger)api_runSummaryRopeSkippingCount {
    return self.hmjson[@"rope_skipping_count"].integerValue;
}

- (NSInteger)api_runSummaryRopeSkippingAvgFrequency {
    return self.hmjson[@"rope_skipping_avg_frequency"].integerValue;
}

-(NSInteger)api_runSummaryRopeSkippingMaxFrequency {
    return self.hmjson[@"rope_skipping_max_frequency"].integerValue;
}

- (NSTimeInterval)api_runSummaryRopeSkippingRestTime {
    return self.hmjson[@"rope_skipping_rest_time"].doubleValue;
}

- (BOOL)api_runSummaryIndoorRunCalibrated {
    return self.hmjson[@"indoor_run_calibrated"].boolean;
}
