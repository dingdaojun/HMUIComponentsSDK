 ...
 *  @param  xiaomiCardName  小米的卡名称
 *
 *  @param  protocolType    协议类型（protocolType ？ 门禁卡 : 公交卡）
 *
 *  @param  acctionType     接收类型 （门禁卡：ISSUE/UPDATE | 公交卡：ISSUE/RECHARGE）
 *
 *  @param  completionBlock 回调方法
 *
 *  @see    http://mifit-device-service-staging.private.mi-ae.net/swagger-ui.html#!/nfc45bus45card45controller/getProtocolsUsingGET
 ...
 *  @param  completionBlock 回调方法
 *
 *  @see    http://mifit-device-service-staging.private.mi-ae.net/swagger-ui.html#!/nfc45bus45card45controller/postProtocolsUsingPOST
 *  @ower   胡志勇
 */
- (id<HMCancelableAPI>)wallet_confirmProtocolWithID:(NSString *)ID
                                       protocolType:(NSString *)protocolType
 ...
 *  @param  completionBlock 回调方法
 *
 *  @see    http://mifit-device-service-staging-nfc.mi-ae.net/swagger-ui.html#!/nfc45access45card45controller/getFareCardInfoListUsingGET
 *  @ower   胡志勇
 */
- (id<HMCancelableAPI>)wallet_acessCardWithQueryType:(NSString *)queryType
                                     completionBlock:(void (^)(NSString *status, NSString *message, NSArray<id<HMServiceAPIWalletaAcessCardInfoProtocol>> *infos))completionBlock;
