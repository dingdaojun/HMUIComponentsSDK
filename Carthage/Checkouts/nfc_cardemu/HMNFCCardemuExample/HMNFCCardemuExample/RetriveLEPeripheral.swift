...
class RetriveLEPeripheral {
    ...
    typealias ScanInfoBlockQueue = ValueSemaphore<HMPeripheralInfomation>
    ...
    static func findPeripheralInfo(_ macAddress: String, timeout seconds: Int = 30) throws -> HMPeripheralInfomation {
        let queue = ScanInfoBlockQueue()
        try LECentralManager.sharedInstance.scanPeripheralsWithServiceUUDIs(nil, nil) { (scanResponse) in
            var scanResult = ScanResult(identifier: scanResponse.peripheral.identifier)
            scanResult.analysisAdvertisement(advertisement: scanResponse.advertisement)
            print("scan mac: \(String(describing: scanResult.macAddress))")
            if scanResult.macAddress == macAddress {
                ...
                queue.push(infomation)
            }
        }
        
        do {
            let info = try queue.pop(timeout: Double(seconds))
            LECentralManager.sharedInstance.stopScan()
            return info
        } catch {
            LECentralManager.sharedInstance.stopScan()
