...
extension PayCloud {
    ...
    public func dealInfoListFromNetwork(city: PayCity, cardNumber: String, nextDate: Date?, limit: Int = 10, callback: @escaping DealListNetworkCallback) {
        PayTransmit.shared.payQueue.async {
            PayTransmit.shared.nfcService.busCard_transactionRecord(withCityID: city.info.code, cardID: cardNumber, type: .consumption, nextTime: nextDate, limit: limit, completionBlock: { (success, msg, dealLists, nextDate) in
                PayTransmit.shared.payQueue.async {
                    guard success else {
    ...
    public func uploadDealList(city: PayCity, cardNumber: String, dealList: [DealInfo], callback: @escaping UploadDealListCallback) {
        PayTransmit.shared.payQueue.async {
            ...
            PayTransmit.shared.nfcService.busCard_uploadTransactionRecord(withCityID: city.info.code, cardID: cardNumber, records: records, completionBlock: { (success, msg) in
                PayTransmit.shared.payQueue.async {
                    if success {
                        callback(nil)
                    } else {
