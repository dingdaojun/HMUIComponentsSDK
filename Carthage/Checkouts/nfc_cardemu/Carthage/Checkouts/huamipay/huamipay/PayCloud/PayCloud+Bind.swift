...
extension PayCloud {
    ...
    public func bindCard(deviceID: String, cardNumber: String, deviceType: Int, city: PayCity, callback: @escaping BindCardCallback) {
        PayTransmit.shared.payQueue.async {
            ...
            PayTransmit.shared.nfcService.busCardsBinding_bind(withDeviceID: deviceID, deviceType: deviceType, card: bindCard, completionBlock: { (success, msg) in
                PayTransmit.shared.payQueue.async {
                    if success {
                        callback(nil)
                    } else {
    ...
    public func unBindCard(deviceID: String, cardNumber: String, callback: @escaping UnBindCardCallback) {
        PayTransmit.shared.payQueue.async {
            PayTransmit.shared.nfcService.busCardsBinding_unbind(withDeviceID: deviceID, cardID: cardNumber) { (success, msg) in
                PayTransmit.shared.payQueue.async {
                    if success {
                        callback(nil)
                    } else {
