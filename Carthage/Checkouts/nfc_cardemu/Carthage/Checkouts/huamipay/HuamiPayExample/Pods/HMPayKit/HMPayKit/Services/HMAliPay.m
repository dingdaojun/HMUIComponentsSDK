...
- (void)parseResult:(NSDictionary *)resultDic {
    ...
    NSError *error = [NSError errorWithDomain:NSStringFromClass(self.class) code:kPayErrorCode userInfo:@{NSLocalizedDescriptionKey:message}];
    if (status != HMPayResultStatusSuccess) {
#ifdef DEBUG
        NSLog(@"ali pay支付失败, status: %ld, message: %@",(long)status, message);
#endif
    }
    self.payResultHandler(status, resultDic, error);
}
