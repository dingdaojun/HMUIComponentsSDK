...
extension PayMICloud {
    ...
    public func queryRelativeCost(_ orderType: PayOrderType, inCity city: PayCity, callback: @escaping HuamipayMICloudProtocol.OrderFeeCallback) {
        ...
        let network = PayTransmit.shared.nfcService.wallet_orderFee(withCityID: city.info.code, orderType: type, xiaomiCardName: city.info.miName) { (state, msg, feeProtocols) in
    ...
    public func generateOrder(_ feeID: String, in city: PayCity, with location: CLLocation?, callback: @escaping HuamipayMICloudProtocol.OrderCallback) {
        let network = PayTransmit.shared.nfcService.wallet_order(withCityID: city.info.code, feeID: feeID, orderType: .openCardAndRecharge, paymentChannel: .alipay, paymentAmount: 0, loction: location) { (state, msg, orderPro) in
            PayTransmit.shared.payQueue.async {
                guard state != PayNetworkState.networkFail.rawValue else {
                    callback(nil, PayError.networkError(msg: msg ?? ""))
                    return
    ...
    public func ordersList(with city: PayCity, callback: @escaping HuamipayMICloudProtocol.OrderListCallback) {
        PayTransmit.shared.payQueue.async {
            let network = PayTransmit.shared.nfcService.wallet_orderList(withCityID: "", xiaomiCardName: city.info.miName, start: Date(), end: Date(), count: Int(INT_MAX), orderCategory: HMServiceWalletOrderCategory.all, completionBlock: { (state, msg, pro) in
                PayTransmit.shared.payQueue.async {
                    guard state != PayNetworkState.networkFail.rawValue else {
    ...
    public func orderInfo(orderNum: String, callback: @escaping HuamipayMICloudProtocol.OrderInfoCallback) {
        PayTransmit.shared.payQueue.async {
            let network = PayTransmit.shared.nfcService.wallet_orderDetail(withOrderID: orderNum) { (state, msg, pro) in
                PayTransmit.shared.payQueue.async {
                    guard state != PayNetworkState.networkFail.rawValue else {
                        callback(nil, PayError.networkError(msg: msg ?? ""))
                        return
