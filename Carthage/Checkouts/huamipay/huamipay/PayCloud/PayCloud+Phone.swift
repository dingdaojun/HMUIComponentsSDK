...
extension PayCloud {
    ...
    public func readUserPhone(callback: @escaping PayCloudProtocol.ReadUserPhoneCallback) {
        PayTransmit.shared.payQueue.async {
            let network = PayTransmit.shared.nfcService.busCardsBinding_retrieve { (success, msg, config) in
                guard success else {
    ...
    public func fetchVerficationCode(for phoneNum: String, callback: @escaping PayCloudProtocol.VerficationCodeCallback) {
        ...
        PayTransmit.shared.payQueue.async {
            let network = PayTransmit.shared.nfcService.busCardsBinding_verifyCaptcha(withPhone: phoneNum, completionBlock: { (success, code, msg) in
                guard success else {
    ...
    public func bindPhone(_ phoneNum: String, verfucatuibCode: String, callback: @escaping PayCloudProtocol.BindPhoneCallback) {
        ...
        PayTransmit.shared.payQueue.async {
            let network = PayTransmit.shared.nfcService.busCardsBinding_boundPhone(phoneNum, verifyCaptcha: verfucatuibCode, completionBlock: { (success, msg) in
                guard success else {
                    callback(PayError.networkError(msg: msg ?? ""))
                    return
                }
