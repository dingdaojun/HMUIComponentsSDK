...
extension PaySNCloud {
    ...
    ///   - callback: 回调
    /// - Returns: Void
    public func fetchProtocol(for city: PayCity, with actionType: PayProtocolActionType, callback: @escaping HuamipaySNCloudProtocol.ProtocolCallback) {
        PayTransmit.shared.payQueue.async {
            let network = PayTransmit.shared.nfcService.wallet_protocol(withXiaomiCardName: city.info.miName, acctionType: actionType.rawValue, completionBlock: { (state, msg, apiProtocol) in
    ...
    public func configProtocol(for identifier: String, callback: @escaping HuamipayMICloudProtocol.ConfigProtocolCallback) {
        PayTransmit.shared.payQueue.async {
            let network = PayTransmit.shared.nfcService.wallet_confirmProtocol(withID: identifier, completionBlock: { (state, msg) in
                guard state != PayNetworkState.networkFail.rawValue else {
