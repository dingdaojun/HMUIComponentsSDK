...
public class PayCityJilin: PayCityBase, CityCardApduProtocol {
    public static var miCardName: String { return "JILIN" }
    public static var aid: Data { return Data.init(hex: "A0000006320101051320004A494C494E") }
    public static var miFetchModel: String { return "SYNC" }
    public static var cityName: String { return "吉林通" } //MOT
    public static var cityCode: String { return "0432" }
    ...
    public func setDefault(_ non: Bool, for city: PayCity) throws {
        try setDefault(non, aid: PayCityJilin.aid)
    }
    ...
    public func isDefault() throws -> Bool {
        return try isDefault(PayCityJilin.aid)
    }
}

extension PayCityJilin {
    /// 余额(分)
    ///
    /// - Returns: Int
    /// - Throws: Error
...
extension PayCityJilin {
    ...
    func select() throws {
        // select
        let selRes = try PayTransmit.shared.transmit(PayCityBase.select(PayCityJilin.aid))
