...
public class PayCitySuzhou: PayCityBase, CityCardApduProtocol {
    public static var miCardName: String { return "SUZHOUTONG" }
    public static var aid: Data { return Data.init(hex: "535558494E2E4D46") }
    public static var miFetchModel: String { return "SYNC" }
    public static var cityName: String { return "苏州公交" }
    public static var cityCode: String { return "0512" }
    ...
    public func setDefault(_ non: Bool, for city: PayCity) throws {
        try setDefault(non, aid: PayCitySuzhou.aid)
    }
    ...
    public func isDefault() throws -> Bool {
        return try isDefault(PayCitySuzhou.aid)
    }
}

extension PayCitySuzhou {
    /// 余额(分)
    ///
    /// - Returns: Int
    /// - Throws: Error
...
extension PayCitySuzhou {
    func readBalance() throws -> Int {
        // bance
        let banRes = try PayTransmit.shared.transmit(Data.init(hex: "805C000104"))
    ...
    func selectAid() throws {
        // select
        let selRes = try PayTransmit.shared.transmit(PayCityBase.select(PayCitySuzhou.aid))
    ...
    func selectADF() throws {
        // ADF
        let selADFRes = try PayTransmit.shared.transmit(Data.init(hex: "00A4000002DF01"))
