...
- (void)jumpToPay:(id<HMPayOrder>)order callBack:(HMPayResultHandler)handler {
    ...
    if (![self isAppInstalled]) {
        NSError *error = [NSError errorWithDomain:@"HMWxPay" code:kPayAppNotInstall userInfo:@{NSLocalizedDescriptionKey:@"应用未安装"}];
        handler(HMPayResultStatusUnInstall, nil, error);
        return;
    }
    if (![WXApi isWXAppSupportApi]) {
        NSError *error = [NSError errorWithDomain:@"HMWxPay" code:kPayNotSupport userInfo:@{NSLocalizedDescriptionKey:@"该版本微信不支持支付"}];
        handler(HMPayResultStatusCancel, nil, error);
        return;
    }
    // register wxAppId.
...
- (void)onResp:(BaseResp *)resp {
    if ([resp isKindOfClass:[PayResp class]]) {
        ...
        NSError *error = [NSError errorWithDomain:@"HMWxPay" code:kPayErrorCode userInfo:@{NSLocalizedDescriptionKey:errorStr}];
        if (payResultStatus != HMPayResultStatusSuccess) {
#ifdef DEBUG
            NSLog(@"微信支付失败, status: %d, message: %@",resp.errCode,errorStr);
#endif
        }
        self.payResultHandler(payResultStatus, errorDic, error);
    }
}
