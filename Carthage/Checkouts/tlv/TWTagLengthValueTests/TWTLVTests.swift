...
class TWTLVTests: XCTestCase {
    ...
    func testDefault() {
        let defaultResponse = "611e4f10a00000000323008698070100000000009f7002070180020006810123"
        if let data = defaultResponse.asciiHexToData() {
            if let tlv = try? TWTLV(data: data) {
                XCTAssert(tlv.tagId == 0x61)
                XCTAssert(tlv.length == 0x1e);
                XCTAssert(UInt64(tlv.value.count) == tlv.length)
                
                print(tlv.printableTlv())
                print("tlv.tagId: \(String(tlv.tagId, radix: 16, uppercase: false))")
                
                var child = tlv.children[0]
                XCTAssert(child.tagId == 0x4f)
                XCTAssert(child.length == 0x10)
                XCTAssert(UInt64(child.value.count) == child.length)
                XCTAssert(child.children.count == 0)
                
                child = tlv.children[1]
                XCTAssert(child.tagId == 0x9f70)
                XCTAssert(child.length == 0x02)
                XCTAssert(UInt64(child.value.count) == child.length)
                XCTAssert(child.children.count == 0)
                
                
                child = tlv.children[2]
                XCTAssert(child.tagId == 0x80)
                XCTAssert(child.length == 0x02)
                XCTAssert(UInt64(child.value.count) == child.length)
                XCTAssert(child.children.count == 0)
                
                child = tlv.children[3]
                XCTAssert(child.tagId == 0x81)
                XCTAssert(child.length == 0x01)
                XCTAssert(UInt64(child.value.count) == child.length)
                XCTAssert(child.children.count == 0)
                
                print("tlv: \(tlv.tlvString())")
                XCTAssert(tlv.tlvString().uppercased() == defaultResponse.uppercased())
            }
        }
    }
	...
	func testInit(){
		if let testData = testTlvStr.asciiHexToData(){
			if let testTlv = try? TWTLV(data: testData){
                ...
                print("tag: \(testTlv.tagId)  length=\(testTlv.length)")
				...
				XCTAssert(UInt64(child.value.count) == child.length)
				XCTAssert(child.children.count == 0)
				print("tag: \(child.tagId)  length=\(child.length)")
